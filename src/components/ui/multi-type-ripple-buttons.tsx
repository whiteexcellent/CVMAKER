...
export const RippleButton: React.FC<RippleButtonProps> = ({
    ...
    const determinedJsRippleColor = useMemo(() => userProvidedRippleColor || 'var(--button-ripple-color, rgba(0, 0, 0, 0.1))', [userProvidedRippleColor]);
    ...
    const dynamicGridHoverStyles = useMemo(() => {
        ...
        for (let r = 0; r < GRID_HOVER_NUM_ROWS; r++) {
            for (let c = 0; c < GRID_HOVER_NUM_COLS; c++) {
                ...
                const selector = variant === 'hover' ? '.hover-variant-grid-cell' : '.hoverborder-variant-grid-cell';
                const target = variant === 'hover' ? '.hover-variant-visual-ripple' : '.hoverborder-variant-visual-ripple';

                nthChildHoverRules += `
          ${selector}:nth-child(${childIndex}):hover ~ ${target} {
    ...
    const finalClassName = `relative rounded-lg px-4 py-2 border-none bg-transparent isolate overflow-hidden cursor-pointer ${disabled ? "opacity-50 cursor-not-allowed pointer-events-none" : ""} ${className}`;
        ...
        <>
            ...
            <button className={finalClassName} onClick={handleButtonClick} disabled={disabled}>
                ...
                {(variant === 'hover' || variant === 'hoverborder') && (
                    <div
                        className={`${variant}-variant-grid-container absolute inset-0 grid overflow-hidden pointer-events-none z-[0]`}
