                    ...
                    <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                        {msg.parts?.map((part, index) => {
                            if (part.type === 'text' && part.text) {
                                return (
                                    <div key={index} className={`max-w-[85%] rounded-2xl p-3 ${msg.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-white dark:bg-[#111] border border-black/10 dark:border-white/10 shadow-sm'}`}>
                            ...
                            const isSuggestContentTool = part.type === 'tool-suggest_cv_content' || (part.type === 'dynamic-tool' && part.toolName === 'suggest_cv_content');

                            if (isSuggestContentTool) {
                                // If the tool call input is finished streaming, we can display the suggestion
                                if (part.state === 'input-available') {
                                    const { field, value } = part.input as { field: string, value: string };
                                    return (
                                        <div key={part.toolCallId || index} className="mt-2 ml-2 p-3 bg-blue-50/50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800/50 rounded-xl w-[85%] shadow-sm">
                                    ...
                                    // Show a thinking state for the specific tool before it fully finishes
                                    return (
                                        <div key={part.toolCallId || index} className="mt-2 ml-2 p-2 bg-muted/30 border border-black/5 dark:border-white/5 rounded-xl w-[85%] animate-pulse">
