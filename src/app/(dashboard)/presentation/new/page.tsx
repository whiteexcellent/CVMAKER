...
import { ArrowLeft01Icon, Tv01Icon } from "@hugeicons/core-free-icons";
...
export default async function NewPresentationPage() {
    ...
    return (
        <div className="flex flex-col h-full bg-zinc-950/50 text-white font-geist selection:bg-cyan-500/30">
            {/* Workspace Header */}
            <div className="sticky top-0 z-20 flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 md:px-8 md:py-6 border-b border-white/5 bg-zinc-950/80 backdrop-blur-xl shrink-0">
                <div className="flex items-center gap-5">
                    <Link href="/presentation" className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/60 hover:text-white hover:bg-white/10 transition-colors">
                        <HugeiconsIcon icon={ArrowLeft01Icon} strokeWidth={1.5} className="w-5 h-5" />
                    </Link>
                    <div>
                        <p className="text-[12px] uppercase tracking-[0.18em] text-white/45 mb-1">Presentation Flow</p>
                        <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2">
                            <HugeiconsIcon icon={Tv01Icon} className="w-6 h-6 text-cyan-400" strokeWidth={1.5} />
                            {dict.generate.presentationTitle}
                        </h1>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8 animate-in fade-in duration-500">
                <div className="max-w-[700px] mx-auto space-y-8 mt-4">
                    <p className="text-[15px] text-white/60 font-light">
                        {dict.generate.presentationDesc}
                    </p>
